	...
	if ($form['id'] == $form_id) {
		...
		if (($email && is_email($email)) && $community_id !== 'fail') {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = 'Error Sending Form ' . $form_id . ' from frunctions file. The query was ' . $post_json . ' The Response was: ' . $error_message;
			} else {
				$message = 'Form ' . $form_id . ' sent to YGL without error using theme functions file.';
			}
